...
void PUnitLayer::UpdateUnit(unsigned int unit_id)
{
	...
	if (node_contents.find(node_index) == node_contents.end()) {
		node_contents[node_index] = new std::unordered_set<unsigned int>();
	}
...
std::vector<std::unordered_set<unsigned int>*> PUnitLayer::Nearby(unsigned int unit_id)
{
	PUnit* current = members.at(unit_id);
	std::vector<std::unordered_set<unsigned int>*> results;
	...
	results.push_back(center_contents);
